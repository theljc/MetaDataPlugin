// UW_AssetInfo.h — 用户控件基类
// 继承 UUserWidget 和 IWidgetInterface，提供自动注册到 Subsystem 的能力
...
#include "UW_BaseDataWidget.generated.h"

struct FInstancedStruct;
/**
 * UUW_AssetInfo — 用户控件基类
 *
 * 继承自 UUserWidget 和 IWidgetInterface。
 * 在 NativeConstruct 中自动注册到 UEUW_WidgetManager 子系统，
 * 在 NativeDestruct 中自动取消注册。
 *
 * 蓝图可以继承此类来创建用户控件，并重写 OnOpen / OnClose 处理初始化和清理逻辑。
 */
UCLASS(BlueprintType, Blueprintable)
class TEST_API UUW_BaseDataWidget : public UUserWidget, public IWidgetInterface_MetaDataPlugin
{
	GENERATED_BODY()
